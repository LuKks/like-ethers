...
module.exports = class JsonRpcProvider {
  ...
  async getBlock (tag, prefetchTxs) {
    ...
    if (block && prefetchTxs) {
      const txs = block.transactions.map(tx => {
        return {
          ...
          chainId: BigInt(tx.chainId, 16),
          signature: {
            r: tx.r,
            s: tx.s,
            yParity: Number(tx.yParity, 16)
          },
    ...
    return {
      ...
      totalDifficulty: BigInt(block.totalDifficulty, 16),
      gasLimit: BigInt(block.gasLimit, 16),
      gasUsed: BigInt(block.gasUsed, 16),
      stateRoot: block.stateRoot,
      receiptsRoot: block.receiptsRoot,
      blobGasUsed: BigInt(block.blobGasUsed, 16),
      excessBlobGas: BigInt(block.excessBlobGas, 16),
      miner: toChecksumAddress(block.miner),
      extraData: block.extraData,
      baseFeePerGas: BigInt(block.baseFeePerGas, 16),
